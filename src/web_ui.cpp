...
String getWebPage(const String& path) {
  ...
  if (path == "/settings" || path == "/settings.html") {
    ...
    html += "<div class=\"mb-4\"><button type=button onclick=scanWiFi() class=\"inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium\">Scan Networks</button>";
    ...
    html += "</form></div>";
    html += "<div class=\"mt-4\"><button type=button onclick=\"fetch('/api/system/clear-wifi',{method:'PATCH'}).then(()=>{alert('WiFi cleared! Refresh page.');window.location.reload();}).catch(e=>alert('Error: '+e))\" class=\"px-4 py-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 text-sm font-medium\">Clear WiFi & Stop Reconnecting</button></div>";
    ...
    #endif
    html += "</main>";
    html += "<script>function escapeHtml(t){return t?t.replace(/\"/g,'&quot;').replace(/'/g,'&#039;'):'';}fetch('/api/system').then(r=>r.json()).then(d=>{document.getElementById('settingsVersion').textContent='FW '+d.firmware_version;});";
    ...
    #endif
    html += "});";
    html += "function scanWiFi(){const list=document.getElementById('wifiList');list.innerHTML='<div class=text-sm text-gray-500>Scanning...</div>';fetch('/api/wifiscan').then(r=>r.json()).then(data=>{if(data.error){list.innerHTML='<div class=text-sm text-red-600>'+data.error+'</div>';return;}const nets=data.networks||[];if(nets.length===0){list.innerHTML='<div class=text-sm text-gray-500>No networks found</div>';return;}const getSignalIcon=(r)=>{if(r>=-45)return'📶 excellent';if(r>=-60)return'📶 good';if(r>=-75)return'📶 fair';return'📶 weak';};const fmtSig=(r)=>r<-90?getSignalIcon(r):getSignalIcon(r)+' ('+r+' dBm)';const selectHTML = '<select class=\"w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm mt-2\" onchange=\"document.getElementById(\'wifiSSID\').value=this.value;\">' + '<option value=\"\" disabled selected>Select a scanned network...</option>' + nets.map(n => '<option value=\"'+escapeHtml(n.ssid||'')+'\">'+escapeHtml(n.ssid||'')+' '+fmtSig(n.rssi)+'</option>').join('') + '</select>'; list.innerHTML=selectHTML;}).catch(()=>{list.innerHTML='<div class=text-sm text-red-600>Scan failed</div>';});}";
    html += "document.getElementById('wifiForm').addEventListener('submit',(e)=>{e.preventDefault();const ssid=document.getElementById('wifiSSID').value;const pwd=document.getElementById('wifiPassword').value;console.log('Saving WiFi:',ssid);fetch('/api/config/wifi',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({wifi:{ssid:ssid,password:pwd}})}).then(r=>{console.log('WiFi save response:',r.status);if(r.ok){document.getElementById('message').innerHTML='<div class=\"mb-4 p-4 bg-emerald-50 text-emerald-700 rounded-lg\">WiFi saved! Restarting...</div>';setTimeout(()=>window.location.href='/',2000);}else{r.text().then(t=>{document.getElementById('message').innerHTML='<div class=\"mb-4 p-4 bg-red-50 text-red-700 rounded-lg\">Error: '+t+'</div>';});}}).catch(e=>{console.error('WiFi save error:',e);document.getElementById('message').innerHTML='<div class=\"mb-4 p-4 bg-red-50 text-red-700 rounded-lg\">Error: '+e.message+'</div>';});});";
    #if MQTT_SETTINGS_UI_ENABLED
